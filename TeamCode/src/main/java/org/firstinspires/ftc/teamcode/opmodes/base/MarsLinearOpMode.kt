 ...
 * `MecanumDrive.forMarsLinear` / `TankDrive.forMarsLinear` (passing
 * [batteryVoltage]) so AUTO bulk is not re-enabled after [initRobot].
 *
 * <p>Call [initRobot] once at the start of `runOpMode()` (after `hardwareMap`
 * is live), create any hardware, then [waitForStart] (requires init), then either:
 *
 * <ul>
 *   <li>`while (nextFrame()) { ... }` — interactive control (sensors each loop; telemetry
 *       published about every [FRAME_TELEMETRY_MS])
 ...
 *
 * <ol>
 *   <li>Returns false if the OpMode is not in the Run phase ([opModeIsActive], which also
 *       yields)
 *   <li>On a telemetry publish frame (see [FRAME_TELEMETRY_MS]): flushes the previous
 *       publish snapshot ([GatedTelemetry.forceUpdate]), opens telemetry writes for this
 *       frame, and queues loop timing into telemetry
 *   <li>On non-publish frames: closes telemetry writes so `addData`/`addLine` are
 *       no-ops, and [dashboardPacket] is a throwaway packet (field overlay is not sent)
 ...
 * </ol>
 *
 * <p>[nextSample] is for dense capture (CSV, FFT, closed-loop steps): stop is polled every
 * call without yielding; cooperative [idle] runs about every [SAMPLE_YIELD_MS];
 * bulk is refreshed every sample; battery is refreshed at most every
 * [SAMPLE_BATTERY_TTL_MS]; loop timing is recorded; telemetry and Dashboard key/value publishes are
 * not.
 *
 * <p>[batteryVoltage] performs a hub ADC read only when the cached value is invalid
 * (battery is not in the bulk packet). [nextFrame] always invalidates so each frame gets a
 * fresh sample; [nextSample] reuses the cache until [SAMPLE_BATTERY_TTL_MS] elapses
 * so dense loops are not ADC-bound. [loopDt] is the last measured interval (seconds) for
 * controllers.
 */
abstract class MarsLinearOpMode : LinearOpMode() {
    companion object {
        /**
         * Max time between cooperative [idle] calls inside [nextSample] (ms). Stop is
         * still checked every sample via [isStopRequested]. `0` disables periodic yield
         * (not recommended for long bursts).
         */
        @JvmField
        var SAMPLE_YIELD_MS = 500

        /**
         * Max age of the cached battery voltage during [nextSample] bursts (ms). Battery is
         ...
         */
        @JvmField
        var SAMPLE_BATTERY_TTL_MS = 100

        /**
         * Minimum interval between telemetry publish frames in [nextFrame] (ms). Matches the
         ...
         */
        @JvmField
        var FRAME_TELEMETRY_MS = 250

        /**
         * Sliding window (seconds) for loop-interval mean/stdev in [loopStats]. Applied on
         * each [nextFrame] / [nextSample] via
         * [RunningIntervalStats.reconfigure].
         */
        @JvmField
        var LOOP_STATS_WINDOW_S = 2.0
    }
    ...
    /**
     * NanoTime of the last ADC read into [batteryV]; meaningful only when `batteryV`
     * is finite.
     */
     ...
     * run at the start of `runOpMode()` — not in a field initializer or constructor (`
     * hardwareMap` is null until then). Subclasses may override to construct drive/mechanisms after
     * `super.initRobot()`. Telemetry writes stay open through init / [waitForStart]
    ...
    protected open fun initRobot() {
        ...
        gatedTelemetry =
            GatedTelemetry(
                MultipleTelemetry(telemetry, packetAccess.dashboardTelemetry),
            )
    ...
    /**
     * Same as [LinearOpMode.waitForStart], but requires [initRobot] first so
     * bulk/battery/telemetry are set up before the OpMode blocks for play. Resets loop-timing stats
     * so [RunningIntervalStats.getElapsed] is from START, not from init. Seeds `
     * lastSampleYieldNs` so the first [nextSample] does not always force an
     ...
     * `</pre>
     *
     * <p>Do not also call [opModeIsActive] as the loop condition (that would yield twice
     * and skip a clean frame boundary). For mid-frame stop checks without a new bulk/ADC cycle, use
     * [isStopRequested].
     *
     * <p>Public so helpers (e.g. `ReversalFeedforwardId`) can advance a frame without living
     * in this package.
     *
     ...
     * gain steps). Prefer [nextFrame] when the loop should update driver-station /
     * Dashboard telemetry.
     *
     * <p>Each call:
     *
     * <ol>
     *   <li>Returns false if the OpMode has not started or stop was requested (
     *       [isStopRequested], no yield)
     *   <li>About every [SAMPLE_YIELD_MS], calls [idle] so the SDK is not starved,
     *       then rechecks stop
     *   <li>Clears Lynx bulk caches; invalidates the battery cache if
     *       [SAMPLE_BATTERY_TTL_MS] has elapsed (or always if TTL is `0`)
     ...
     * is invalid; later calls reuse the cached value until the next invalidation.
     * [nextFrame] invalidates every frame; [nextSample] invalidates when
     * [SAMPLE_BATTERY_TTL_MS] elapses. Pass to the drive factory: `
    ...
    /**
     * Last measured interval between [nextFrame] / [nextSample] calls (seconds),
     * or [Double.NaN] before the second advance. Same value as
    ...
    /**
     * Dashboard packet for field overlay (and related packet puts). After [nextFrame]: the
     * live Dashboard packet on publish frames, or a throwaway packet on non-publish frames (draws
     * are discarded and never sent). Null before the first frame and during [nextSample]
     * bursts. Prefer [isTelemetryPublishFrame] if draw work itself should be skipped.
     */
    protected fun dashboardPacket(): TelemetryPacket? {
        return packet
    }
     ...
     */
    protected fun isTelemetryPublishFrame(): Boolean {
        return robotInitialized && gatedTelemetry.isOpen()
    }
