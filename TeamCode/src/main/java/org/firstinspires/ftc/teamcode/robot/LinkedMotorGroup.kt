...
/**
 * A group of mechanically coupled motors that act as one. Power and enable/disable are fanned out
 * to every motor; read operations come from the first (primary) motor. Closed-loop modes (velocity,
 * position) are not supported — tandem motors running independent closed-loop controllers would
 * fight each other.
 */
class LinkedMotorGroup(hardwareMap: HardwareMap, vararg configs: MotorConfig) :
    ...
    override fun setMode(mode: DcMotor.RunMode) {
        if (mode == DcMotor.RunMode.RUN_USING_ENCODER || mode == DcMotor.RunMode.RUN_TO_POSITION) {
            throw UnsupportedOperationException(
                "LinkedMotorGroup does not support closed-loop mode $mode"
            )
        }
    ...
    override fun setTargetPositionTolerance(tolerance: Int) {
        throw UnsupportedOperationException(
            "LinkedMotorGroup does not support setTargetPositionTolerance"
        )
    }
    ...
    override fun setVelocityPIDFCoefficients(p: Double, i: Double, d: Double, f: Double) {
        throw UnsupportedOperationException(
            "LinkedMotorGroup does not support setVelocityPIDFCoefficients"
        )
    }

    override fun setPositionPIDFCoefficients(p: Double) {
        throw UnsupportedOperationException(
            "LinkedMotorGroup does not support setPositionPIDFCoefficients"
        )
    }
