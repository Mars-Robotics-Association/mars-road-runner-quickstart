 ...
 * and settling. Starting from a conservative bandwidth, it raises {@code omega_n} by {@link
 * #GROWTH} each time the response stays clean, and stops at the last clean setting once overshoot
 * or oscillation appears — the classic "turn it up until it rings, then back off," automated. Each
...
public final class FeedbackGainTuner extends LinearOpMode {
    ...
    private void runMecanum() {
        ...
        double[] gains = {
            MecanumDrive.PARAMS.axialGain, MecanumDrive.PARAMS.lateralGain,
                    MecanumDrive.PARAMS.headingGain,
            MecanumDrive.PARAMS.axialVelGain, MecanumDrive.PARAMS.lateralVelGain,
        ...
        AxisResult heading =
                searchAxis(
                        ...
                        tauAxial,
                        mecanumStepTest(
                                drive, gains, 2, new Pose2d(0, 0, Math.toRadians(STEP_DEGREES))));
        if (heading.ok) {
    ...
    private void runTank() {
        ...
        AxisResult turn =
                searchAxis("turn", tau, tankTurnStepTest(drive, Math.toRadians(STEP_DEGREES)));
    ...
    private AxisResult searchAxis(String name, double tau, StepTest test) {
        ...
        for (int iter = 1; iter <= MAX_ITERS && opModeIsActive(); iter++) {
            ...
            if (oscillatory) {
                ...
                note =
                        String.format(
                                "ok: clean at posGain %.2f (overshoot %.0f%%)",
