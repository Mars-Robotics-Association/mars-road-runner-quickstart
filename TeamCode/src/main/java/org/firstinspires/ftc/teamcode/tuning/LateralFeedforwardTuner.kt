...
/**
 * Automatic on-robot identification of the anisotropic (strafe) feedforward constants `lateralKS` /
 * `lateralKV` / `lateralKA` for a mecanum drive.
 *
...
class LateralFeedforwardTuner : MarsLinearOpMode() {
    companion object {
         ...
         */
        @JvmField var HEADING_GAIN = 1.2

        /** Heading-rate damping (power per rad/s). */
        @JvmField var HEADING_VEL_GAIN = 0.08

        /** Max |yaw power| blended into the strafe command. */
        @JvmField var HEADING_MAX_CORR = 0.35
    ...
    override fun runOpMode() {
        require(TuningOpModes.DRIVE_CLASS == MecanumDrive::class.java) {
            val kind =
                if (TuningOpModes.DRIVE_CLASS == TankDrive::class.java) "tank" else "non-mecanum"
            "LateralFeedforwardTuner is mecanum-only; a $kind drive has no lateral motion."
        }
