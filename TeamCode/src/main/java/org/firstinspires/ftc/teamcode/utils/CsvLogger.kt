 ...
 * buffered in memory and written in batches: a [FileWriter] is opened once per
 * [flush], never once per [row], so high-rate logging from a control loop pays disk I/O in
 * occasional bursts instead of stalling the loop on every sample. Writing is best-effort — an
 * [IOException] is counted ([failureCount]) and logged, never thrown, so a full disk
 ...
 * filename used across runs accumulates rows under a single header, while a [timestamped]
 * per-run filename gets a fresh file with its own header.
 *
 * <p>Values passed to [row] are stringified with [String.valueOf]; doubles therefore
 * serialize at full round-trip precision (e.g. a `t_ms` timestamp keeps sub-microsecond
 * resolution), and `NaN` is written literally. This is deliberately better for offline
 * analysis than a fixed `%.3f`-style format, which would silently truncate timestamps.
 *
...
class CsvLogger(filename: String, private val header: String) : AutoCloseable {
     ...
     * on an [IOException] the buffer is still cleared (so a transient failure doesn't wedge
    ...
    companion object {
        ...
        fun timestamped(prefix: String): String {
            val stamp =
                java.text.SimpleDateFormat("yyyyMMdd_HHmmss", Locale.US).format(Date())
