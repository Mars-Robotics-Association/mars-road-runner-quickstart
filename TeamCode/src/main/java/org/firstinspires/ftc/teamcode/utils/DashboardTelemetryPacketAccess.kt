 ...
 * `FtcDashboard.TelemetryAdapter`. The [Field] reference is cached at construction time to avoid
 * repeated reflection lookups.
 */
class DashboardTelemetryPacketAccess {
    @JvmField val dashboardTelemetry: Telemetry = FtcDashboard.getInstance().telemetry
    private val currentPacketField: Field

    /**
     * Constructs an accessor for the given dashboard telemetry instance.
     *
     * @throws RuntimeException if the field cannot be found (e.g., wrong telemetry type or API
     *   change).
     */
    init {
        try {
            ...
            throw RuntimeException(
                "Failed to find currentPacket field. Is this a dashboard Telemetry?",
                e,
            )
        }
    }
     ...
     *
     * <p>This packet is reset after each `telemetry.update()` call, so you should access it each
     * loop iteration before calling update.
     *
