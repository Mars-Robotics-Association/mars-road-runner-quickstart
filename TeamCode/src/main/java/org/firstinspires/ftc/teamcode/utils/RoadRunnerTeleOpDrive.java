...
public class RoadRunnerTeleOpDrive {
     ...
     */
    public static PoseVelocity2d toDrivePowers(boolean fieldCentric, double lateralSpeed, double axialSpeed, double turnSpeed, double speedFactor, Rotation2d heading) {
        Vector2d input = new Vector2d(
                -axialSpeed,
                -lateralSpeed).times(speedFactor);
        if (fieldCentric) {
     ...
     * @return a PoseVelocity2d that represents velocities in the weird RR coordinate system. Ready to pass directly into setDrivePowers
     */
    public static PoseVelocity2d toDrivePowers(boolean fieldCentric, Gamepad gamepad, double crouchSpeed, Rotation2d heading, boolean forceCrouch) {
        return toDrivePowers(
                fieldCentric,
                gamepad.left_stick_x,
                gamepad.left_stick_y,
                gamepad.right_stick_x,
                gamepad.left_stick_button || forceCrouch ? crouchSpeed : 1.0, // analogous to "crouch" in Halo
                heading
        );
    }
}
