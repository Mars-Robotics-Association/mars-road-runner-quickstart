...
:: uncommitted parent sources). Warn and ask before continuing so a dirty pin
:: is an intentional choice, not a silent mismatch.
git -C "!SCRIPT_DIR!" submodule foreach --recursive "git diff --quiet && git diff --cached --quiet" >nul 2>&1
if errorlevel 1 (
    echo WARNING: one or more submodules have uncommitted local changes.
    echo          Prefetch builds the working tree; the APK will include those edits
    echo          and will not match the clean pin commit^(s^).
    git -C "!SCRIPT_DIR!" submodule foreach --recursive "git status -s"
    echo.
    set /p DIRTY_CHOICE="Continue with dirty submodules? [Y/N] "
    if /i not "!DIRTY_CHOICE!"=="Y" (
        echo Aborted. Commit, stash, or discard submodule changes, then retry.
        exit /b 1
    )
    echo Continuing with dirty submodules ...
)
